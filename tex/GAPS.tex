...
\begin{figure}[H]
    ...
    \caption[GAPS experiment]{Left figure shows \ac{GAPS} payload design while right figure shows the actual payload hanging from the flight vehicle at McMurdo Station}
    \label{fig:GAPS_antarctic}
\end{figure}

As mentioned in Section.~\ref{chap:intro:GAPS}, \ac{GAPS} is an Antarctic balloon mission designed to search for low-energy ($< 0.25 GeV/N$) cosmic-ray antinuclei that launched its first science flight in the austral summer of 2025. It is designed to precisely measure the flux of low-energy cosmic-ray antideuterons, antiprotons, and antihelium. \ac{GAPS} has been contributing to its science community since the early 2000s, with several key project milestones:
\begin{itemize}
    \item 2002--2004: First idea \cite{HAILEY2004122} and beam test \cite{Hailey_2006}.
    \item 2012: pGAPS balloon flight \cite{MOGNET201424, J_E_Koglin_2008}. 
    \item 2018--2019: Science flight detector fabrication \cite{Rogers_2019, KOZAI2022166820, scotti2019frontendelectronicsgapstracker, PEREZ201812, a5c7b362e95a41df8a83d91fe2100f18, OKAZAKI201820}.
    \item 2020--2025: \ac{GAPS} science flight payload assembly and testing\cite{10168972, Tiberio_2025, Aoyama:2025S0}.
    \item 2025--2026: \ac{GAPS} first \ac{LDB} flight launched on December 16, 2025, at 5:37 NZDT, see Fig.~\ref{fig:GAPS_antarctic} and Fig.~\ref{fig:GAPS_launch}.
    \item 2026--: Data analysis and preparation for the second flight.
\end{itemize}
...
\begin{figure}
    ...
    \noindent\raggedright 
    \caption[GAPS launch on December 16th 2025 at 5:37 am NZDT from McMurdo Station]{GAPS launch on December 16th 2025 at 5:37 am NZDT from McMurdo Station. Left photo shows NASA \ac{LDB} inflating in preparation for launch. The middle photo shows the moment when the \ac{GAPS} payload left the launch vehicle. The right photo shows the \ac{GAPS} payload ascending into the stratosphere against a clear sky.}
    \label{fig:GAPS_launch}
\end{figure}

Because my primary contributions to \ac{GAPS} began in 2021, this thesis focuses mainly on the \ac{GFP} and the first \ac{GAPS} science flight.
The \ac{GAPS} first science balloon flight design is displayed in Fig.~\ref{fig:GAPS_antarctic}. The instrument is composed of two primary detector subsystems: a \ac{TOF} system for particle timing and a high-resolution tracker (\ac{Si(Li)}) for energy measurement and charged-particle tracking. The \ac{TOF} system features inner and outer layers of EJ-200 plastic scintillator paddles. The outer \ac{TOF} consists of a horizontal plane above the rest of the detector, referred to as the umbrella, and four lateral vertical walls, referred to as the cortina. The inner \ac{TOF} is a cube that surrounds the tracker system on the top, bottom and lateral sides. All scintillators are $6.35~mm$ thick and $16~cm$ wide, with a length between 1.1 and 1.8 m. Each paddle is read out with \ac{SiPM} (Hamamatsu S13360-6050VE) on both ends. The tracker system is made of 1008 \ac{Si(Li)} detectors arranged in 7 planes evenly spaced by $10~cm$ \cite{PEREZ201812, KOZAI2019162695, KOZAI2022166820, SAFFOLD2021165015, 10168972}.
Each tracker plane consists of $6\times6$ modules, where each module contains 2×2 detectors. Each detector has a cylindrical shape of $ 10~cm$ diameter and $ 2.5~mm$ thickness and it is segmented into eight strips of equal area. The required operational temperature of $ \sim -40^\circ C$ is achieved with an \ac{OHP} system \cite{doi:10.1142/S2251171714400042, doi:10.1142/S2251171717400062, OKAZAKI201820, FUKE2023168102}. We have developed a noise model for the \ac{Si(Li)} detector system to optimize the operating temperature and peaking time of the readout electronics \cite{Rogers_2019,ARAMAKI201290}
\begin{equation}
...
The readout is performed with a dedicated \ac{ASIC} which provides the required dynamic range between $ \sim10~keV$ and $ \sim100~MeV$ \cite{9564082, RE2023167617, 10330133}. The resulting energy resolution of the \ac{Si(Li)} detector is better than $ 4~keV$ for measurements around $ 60~keV$, fulfilling the requirement for X-ray discrimination \cite{Tiberio_2025}.


In addition to the detector subsystems, an integrated thermal system is located at the center of \ac{Si(Li)} tracker and dissipates heat through the radiator panels on one side of the payload, see Section.~\ref{chap:GAPS:thermal} for details. The other side of the payload carries 16 solar panels that provide power during flight and help balance the payload \ac{COM}. On top of the payload, we installed Iridium and TDRSS antennas on the BOOM to achieve the best communication coverage. The electronics are housed in the \ac{EBay} at the base of the payload.
...
\begin{figure}
    ...
    \caption[GAPS detector module]{GAPS detector module. Left figure shows \ac{GAPS} \ac{Si(Li)} module which contains 4 detectors, a custom 32-channel ASIC, and a front-end board. Right figure shows a close view of each detector which are wire-bounded from each 8 channels to the front-end board.}
    \label{fig:GAPS_detector}
\end{figure}
This section reflect work and development that has being done mainly in Columbia University and \ac{JAXA}.

As mentioned in previous section, \ac{GAPS} uses \ac{Si(Li)} detectors as its main tracker and calorimeter. Each detector is $10~cm$ in diameter and $2.5~mm$ thick, segmented into 8 strips. A total of 1008 detectors are arranged in 7 layers to form the tracker system. Each layer contains 36 modules, and each module consists of 4 detectors, as shown in Fig.~\ref{fig:GAPS_detector}. The detectors are fabricated using a novel lithium drifting method to create a thick intrinsic layer with high energy resolution at a relatively high operating temperature of $-40~^\circ C$. The fabrication process is illustrated in Fig.~\ref{fig:GAPS_detector_fab}. This thesis summarizes the key fabrication steps used to achieve the high-quality \ac{Si(Li)} detectors. For the detailed description, readers can refer to \cite{KOZAI2019162695}.
...
\begin{figure}
    ...
    \caption[Lithium drift for \ac{GAPS} \ac{Si(Li)} detector fabrication]{Lithium drift for \ac{GAPS} \ac{Si(Li)} detector fabrication \cite{KOZAI2019162695}. Profiles of the applied bias voltage (top), heater output (middle), and leakage current (bottom) during the drift of a sample detector. The inset plot in each panel displays each parameter's variation in the first 3 hours of the drift, as the voltage is increased step-wise to the 600 V set-point.}
    \label{fig:Li_drift_stage}
\end{figure}
\noindent A uniform Li-drift is achieved by our special \ac{GAPS} drift apparatus, which has been custom-designed for large-area \ac{Si(Li)} detectors. We found that retaining a thin undrifted layer on the p-side effectively suppresses the leakage current. The radial uniformity of the growth of the drifted region during Li drift is key to realizing this thin, uniform undrifted layer. Li drifting is performed in a custom drift apparatus consisting of an electrically grounded heater plate, a pressure contact for applying a bias voltage, a \ac{RTD}, and a controller. The Si wafer is placed on the heater plate with the n-side facing upward, while the p-side remains in contact with the grounded heater plate. The pressure contact and RTD are connected to the n-electrode to apply a positive bias voltage and monitor the wafer's temperature. The controller then automatically executed the sequence described below. Fig.~\ref{fig:Li_drift_stage} depicts an example of the bias voltage, heater output, and leakage current during the $\approx110$ hours of the drifting routine. Our sample detector is a $10~cm$ \ac{Si(Li)} detector. The voltage is increased step-wise, in 100 V intervals every 30 min, to prevent rapid increase of the leakage current. The insets in each panel of Fig.~\ref{fig:Li_drift_stage} display the variations over the first 3 hours of the drift, as the voltage is ramped up to the set-point of 600 V. In the first panel, open arrows indicate the timing of the voltage steps. In the final panel, the leakage current shows a step-like increase corresponding to each voltage step. As Li drifts toward the p-side and the depletion layer expands from the n-side, the leakage current gradually increases. The Joule heat generated by the leakage current also increases. As shown in the middle panel, heater output is automatically reduced to compensate for the Joule heat and keep the wafer at $100^\circ~C$. At the end of the drift, the depletion layer approaches the p-side. At this point, the leakage current rapidly increases, the wafer temperature exceeds $100^\circ~C$ due to the Joule heat, and the heater output decreases to zero. The bias voltage is automatically turned off, i.e., the Li drift is terminated, either when the leakage current reaches 25 mA or when the heater output becomes zero. The wafer is then allowed to naturally cool to room temperature. \cite{KOZAI2019162695}. Under a bias voltage as high as 600 V, the depletion layer expands slightly toward the p-side beyond the "i-p junction" formed between the drifted and undrifted layer. Thus, despite the steep increase in leakage current, a thin undrifted layer is retained on the p-side of our \ac{Si(Li)} detectors after the drifting process. The drift depth, W, increases with drift time, T, as \cite{osti_4532160}
...
\begin{equation}
    D = \mu_L k_B T / q
\end{equation}
\noindent with the elementary charge q and the Boltzmann constant $ k_B $. For \ac{GAPS} drift parameters, we obtain $t\approx100h$ to obtain a drifted depth $W\approx2.2mm$ ($90\%$ of the overall $2.5~mm$ thickness). This calculated result is comparable to \ac{GAPS}actual drift time displayed in Fig.~\ref{fig:Li_drift_stage}. In the case that the Li drift is unexpectedly terminated before $\sim 100h $, we resume the drift sequence to ensure detector quality. The higher bias voltage or temperature would reduce the fabrication time while at the same time will generate more hole-electron pairs, which attract Li-compensation, hence disturbing the ideal Li distribution, which should only compensate for acceptors \cite{goulding1966semiconductor}. The optimum parameters for bias voltage $600V$ and temperature $100^\circ~C$ here were experimentally optimized and determined. In conventional \ac{Si(Li)} fabrication, the undrifted layer near the p-side is removed and replaced with a metal contact, such as evaporated gold, to form a Schottky barrier. For \ac{GAPS}, however, this conventional approach resulted in excessively large leakage current at $-35^\circ~C$. with this conventional method. But if we kept the $ 100\mu m$ undrifted layer, the leakage current was significantly suppressed, see Fig.~\ref{fig:leakage_current_comparison}. With conventional way of polishing the p-side region, they are used as windows for low-energy X-ray detection \cite{lyman1989x, rossington1991si, cox2005improvement}, such detectors adopted Schottky barrier contacts to minimize the p-side insensitive layers while suppressing bulk leakage currents. \ac{GAPS} aims to detect X-rays with energies higher than $20~keV$, the $\sim 100\mu m$ insensitive layer is acceptable.

\begin{figure}
    \centering
    \includegraphics[width=0.8\linewidth]{fig/leakage_current_comparison.jpg}\\
    \caption[Leakage current comparison between p-side finishing methods]{Leakage current comparison between p-side finishing methods. Under $ -35^\circ~C $ condition, for the same detector before and after removing the undrifted layer on p-side, the leakage current shows significant difference.}
...
\begin{figure}
    ...
    \caption[GAPS payload and detection concept]{GAPS payload and detection concept. Left figure shows \ac{GAPS} payload design while right figure shows the detection concept of \ac{GAPS}. An incoming low-energy antinucleus (antiproton, antideuteron, antihelium) is slowed down by the \ac{TOF} and \ac{Si(Li)} layers until it stops in the tracker volume. It then forms an exotic atom by replacing a shell electron, de-excites by emitting characteristic X-rays, and finally annihilates with the nucleus producing a star of pions and protons.}
    \label{fig:GAPS_detection_concept}
\end{figure}

Once a charged particle enters the active detector volume, the \ac{TOF} system records its timing and position information. Low-energy particles that satisfy the trigger selection are then recorded in the \ac{Si(Li)} tracker, where the detector strips serve both as tracking elements and as a calorimeter. Ordinary charged cosmic rays such as protons and helium nuclei typically leave a primary track with limited secondary activity, whereas captured antiparticles produce a distinct annihilation signature with multiple secondaries emerging from the stopping point. For normal standard model \ac{CRs} like proton and helium, they will leave an primary track with limited amount of charged secondary generated. While if there is an antiproton or antideuteron captured, the annihilation vertex will be constructed by adding all the charged secondary to construct an unique detector response. 

Taking antideuteron detection, a primary goal of \ac{GAPS}, as an example: The key point of the experiment is to identify antideuterons from all other type of charged particles. Multiple background-rejection channels must be considered. 

A large fraction of the expected background consists of ordinary charged cosmic-ray particles, particularly protons and deuterons. Their energy deposition is correlated with their incident kinetic energy and can be constrained using the \ac{TOF} system. Compared with primary antiparticles that stop and annihilate in the tracker, these events produce substantially less deposited energy and a much less distinctive secondary-particle signature.

Another major background arises from antiprotons. Because cosmic antiprotons are expected to stop and annihilate in the tracker, they are more difficult to reject using only \ac{TOF} information and deposited energy. However, antiprotons and antideuterons have different stopping powers and therefore different penetration depths for the same incident kinetic energy. In addition, \ac{GAPS} exploits the exotic-atom technique: a low-energy antinucleus slows down in the \ac{TOF} and \ac{Si(Li)} layers, stops in the tracker, forms an exotic atom, emits characteristic X-rays during de-excitation, and finally annihilates to produce a star of pions and protons. Because an antideuteron contains one additional antinucleon relative to an antiproton, its annihilation typically produces a higher secondary-particle multiplicity, see Fig.~\ref{fig:GAPS_detection_concept}. 

\section{GAPS Functional Prototype (GFP)}
\label{chap:GAPS:GFP}
To confirm \ac{GAPS} functionality for the full payload, we have proposed and tested a smaller scale assembly called \ac{GAPS} functional prototype (\ac{GFP}). \ac{GFP} uses 3 layers of detector modules with $ 2\times6 $ modules installed on each layer. For the \ac{TOF} system, instead of reproducing the full umbrella, cortina, and inner-cube geometry, the \ac{GFP} used two large panels, referred to as the top \ac{TOF} and middle \ac{TOF}, to trigger and reconstruct incoming charged particles. A smaller-scale thermal system \ac{OHP} / \ac{MCHP} is also designed and fabricated. The remaining electronics were kept flight-representative so that the \ac{GAPS} concept could be validated at the system level to prepare for readiness for the first science flight during the pandemic period. This section discusses the construction, design of the GFP. The performance of each individual sub-system and result including the thermal performance, the study of noise in the \ac{Si(Li)} tracker, the measurement of the energy spectrum of minimum ionizing particles and the reconstruction of muon tracks are discussed in Section.~\ref{chap:GAPSresult:GFP}. 

\begin{figure}
    \centering
    \includegraphics[width=0.9\linewidth]{fig/GFP_system_diagram.png}\\
    \caption[GFP \ac{TOF} system diagram]{GFP \ac{TOF} system diagram}
...
\begin{figure}
    ...
    \caption[GAPS \ac{GFP} setup at Bates lab]{GAPS \ac{GFP} setup at Bates lab. Overview of the \ac{GFP} setup: \ac{Si(Li)} tracker, \ac{TOF} and Thermal. Instruments which are not shown in the photo: tracker power system (on the bottom of electronic rack) and tracker DAQ computer.}
...
\begin{figure}
    ...
    \caption[A \ac{GFP} \ac{Si(Li)} detector module (a top aluminized polypropylene window is not shown)]{A \ac{GFP} \ac{Si(Li)} detector module (a top aluminized polypropylene window is not shown).}
...
The \ac{GAPS} \ac{Si(Li)} detector is $10~cm$ in diameter and $2.5~mm$ in thickness, segmented into eight strips and a guard ring. For both the flight and GFP, every four \ac{Si(Li)} detectors are grouped together into one module. Fig.~\ref{fig:GFP_detector} shows the module geometry and its components, which are the same for \ac{GFP} and flight instrument.

An aluminum module frame provides the mechanical, electrical, and thermal interface for the detectors. The frame is $9.5'' \times 9.5''$ and $1/8''$ thick. Four holes with the diameter of $\phi = 9.2~cm$ are cut out to host four \ac{Si(Li)} detectors and a few small holes to interface with connectors on the front-end board. Each detector is rigidly mounted into place using several fluorosilicone O-rings and G10 clamps, protecting the detectors from shock loads at takeoff and cut-down. The detector guard ring is in direct contact with the aluminum frame, electrically connecting the guard ring, the frame, and the module windows to analog ground. Two corners of the frame are machined to interface with the thermal system and one of them is used to mount the cooling collar (the odd and even layer uses a different corner), which transfers heat to the OHP system. In addition, two purge fittings are mounted to the aluminium frame in order to maintain good air conductance between two adjacent modules in the same row. During operation of the GFP, as well as the on-ground testing for the \ac{GAPS} full payload, dry nitrogen gas is flushed inside the modules via these purge fittings in order to protect the \ac{Si(Li)} detectors against the environmental humidity.
...
\begin{figure}
    ...
    \caption[Schematic diagram of the \ac{GFP} power system]{Schematic diagram of the \ac{GFP} power system.}
    \label{fig:GFP_power_schematic}
\end{figure}

The \ac{GFP} power system has the same design as the flight system, but the crate only hosts three pairs of HV and LV cards and can be extended for ten pairs for flight. Each HV card contains 18 individual channels and each channel can supply the HV bias with the maximum voltage of $ 300~V $ and an accuracy of $1~V$. Every 6 channels from one HV card are grouped into a single connector and serve for one \ac{Si(Li)} row, while each HV channel can be individually turned on and off, and operated with a ramp-up rate of $ <5~V/s $. In addition, each HVPS channel can support up to $ 6~\mu A $ leakage including the current from the power system itself, which allows us to bias the \ac{Si(Li)} module with the leakage up to $ \sim 3~\mu A $ at the operation temperature $ \sim-40^\circ~C $. The LVPS provides four different voltages to the ASIC front-end and back-end electronics with a resolution of $ 1~mV $: $ 2.8~V $ for ASIC analog signal, $ 2.8~V $ for ASIC digital signal, $ 3.3~V $ for ASIC calibration electronics, and $ 3.3~V $ for back-end DAQ via the interface board. The 6 modules in a row share the four LV rails with the total power consumption of $ \sim3~W $. During the operation, the software is designed to monitor the leakage current from HVPS with a precision of $ 1-2~nA $ per channel, voltages and current draws from LVPS with an accuracy of $ 1~mV $ and $ 0.7~mA $, as well as the temperatures of the power cards. These measurements are sent to the flight computer for the system control.

\begin{figure}
    \centering
    \includegraphics[width=1\linewidth]{fig/GFP_module_row.png}
    \caption[\ac{GFP} \ac{Si(Li)} detector electronics connection in the row configuration]{\ac{GFP} \ac{Si(Li)} detector electronics connection in the row configuration.}
    \label{fig:GFP_module_row}
\end{figure}
Fig.~\ref{fig:GFP_module_row} illustrates the front-end and back-end electronics connection for one row at the \ac{GFP} setup. Comparing to the flight instrument, the \ac{GFP} electronics connection only adds two patch boards to adapt the connection between the ERNI connection between the \ac{Si(Li)} module and the interface board (IF board). The back-end DAQ communicates with ASICs in the row scheme through SPI. An IF board is attached at the beginning of the row to connect the \ac{Si(Li)} row and back-end electronics (back-end DAQ and low-voltage supplies). For the \ac{GAPS} flight, the IF board is directly mounted on the row via a flex cable which is fabricated with two ERNI connectors. However, the flex cable is not ready during the \ac{GFP}. So a patch board and ribbon cable is made as the solution. Six modules in a row are chained with flex-rigid cables which are made of Kapton ribbon cable with ERNI connector at two ends. At the other end of the row, a terminator with the resistance of $ 100 \Omega $ is mounted to terminate the row connection. Each \ac{Si(Li)} module in the row is assigned with a unique physical address, numbering from 0 (beginning) to 5 (end) regarding the distance between the module and the readout board. In addition, a separate HV cable is attached to the row to provide the HV bias to each \ac{Si(Li)} module individually.

\subsection{GFP Thermal System}
\label{chap:GAPS:GFP:thermal}
Due to both \ac{GAPS} project timeline and the pandemic situation, the \ac{GFP} thermal system was designed, fabricated, and tested before the flight system was assembled. The \ac{GAPS} thermal system is tightly integrated with the detector system, making parallel preparation difficult. Then the \ac{GFP} thermal system is designed to validate the novel \ac{OHP} cooling technique for \ac{GAPS} \ac{Si(Li)} tracker before the full payload is ready, see Section.~\ref{chap:GAPS:thermal:OHP}. The \ac{GFP} thermal system is designed to be as close as possible to the flight thermal system in order to provide a representative validation of the \ac{OHP} technique. This section discusses the design of the \ac{GFP} thermal system. The concept of \ac{OHP} and \ac{MCHP} will be discussed in Section.~\ref{chap:GAPS:thermal:OHP} while the performance of the \ac{GFP} thermal system using \ac{R23} as coolant is presented in Section.~\ref{chap:GAPSresult:GFP:thermal}.

\begin{landscape}
\begin{figure}
    \centering
    \includegraphics[width=1\linewidth]{fig/GFP_thermal.jpg}
    \caption[The \ac{GFP} thermal system design, with only one \ac{Si(Li)} row shown (left) and the setup (right)]{The \ac{GFP} thermal system design, with only one \ac{Si(Li)} row shown (left) and the setup (right).  Dual-phase trifluoromethane (\ac{R23}) filled in the reservoir and the rest of the swaglok tubings. With heat generated from the detector and electronics on the right side, liquid R23 will vaporize and drive the fluid upward to return to the top and the radiator on the left. Then the R23 will dissipate heat and become liquid again, drop to the bottom of the radiator due to gravity. This will keep the cycle flowing inside each loop.}
...
\begin{figure}
    ...
    \caption[GFP thermal design with each row of detector tagged]{GFP thermal design with each row of detector tagged. In this photo, two rows of detector L1C2 and L2C2 are tagged. As the time of this picture was taken, only these two rows of detectors are installed and the result from Section.~\ref{chap:GAPSresult:GFP:thermal}, Fig.~\ref{fig:GFP_thermal_result} are based on this configuration dated back on December 1st, 2021.}
    \label{fig:GFP_thermal_line}
\end{figure}
\end{landscape}
\subsubsection*{System requirements}
The \ac{GFP} thermal system was required to cool the \ac{Si(Li)} detectors to their nominal operating temperature of about $-40^\circ~C$ while also validating the passive \ac{OHP} cooling concept that doesn't have any active compressor to reduce the total weight of the payload. For balloon experiments, minimizing total payload mass is important for achieving higher float altitude, detail information discussed in Section.~\ref{chap:intro:balloon}. Details of \ac{OHP}/\ac{MCHP} are discussed in Section.~\ref{chap:GAPS:thermal:OHP}. There is no strict weight requirement for GFP because it operates on the ground, but the goal of system validation makes the design of the \ac{GFP} thermal system as close to flight thermal system as possible.
...
As discussed in Section.~\ref{chap:GAPS:detectors}, \ac{Si(Li)} detectors perform best under $\sim-40^\circ~C$, therefore require the \ac{GAPS} thermal system to cool the tracker to below $ \sim-40^\circ~C $ during the flight, with a designed cooling power $ \sim300~W $. As the payload shows in Fig.~\ref{fig:GAPS_antarctic} and the thermal design schematic shows in Fig.~\ref{fig:GAPS_flight_thermal}, \ac{GAPS} thermal system is designed and integrated closely with detector cube, with two major components, \ac{MCHP}/\ac{OHP} system and flight insulation package.
...
\caption[GAPS flight instrument thermal design schematic]{GAPS flight instrument thermal design schematic. Conceptual diagram of the \ac{GAPS} instrument. To cool the \ac{Si(Li)} tracker consisting of $ >1000 $ detectors, the heat to be removed is transported to a radiator, which is located outside the \ac{TOF} system and pointed to the anti-solar azimuth direction, and then dissipated to space \cite{FUKE2023168102}.}
...
GAPS flight insulation used Styrofoam Brand XPS Insulation from DUPONT \cite{dupont_styrofoam}. The major advantage is the large resistance value of the material, which enables comparable thermal performance at reduced thickness, see chart.~\ref{table:styrofoam}. We have developed a series of styrofoam pieces that covers the cold part of OHP system to a minimum $ 4~'' $ thickness to maintain thermal stability, see table.~\ref{table:styrofoam_list}.

One example styrofoam piece GAPS-RAD-MEC-049\_Foam\_Section\_33 is shown in Fig.~\ref{fig:foam_33}. \textbf{(a)} Shows the design for this styrofoam piece which goes on top of the OHP heat transaction area. Small group of grooves are formed to divide all 36 loops of OHP tubings from tracker to radiator. \textbf{(b)} shows a 3D model of this particular piece. \textbf{(c)} shows the actual piece that I made with styrofoam and router. This design minimizes heat leakage from the tubing by cover all of the cold area. The fabrication of these foams are essential for both ground testing and flight while especially crucial for ground testing since we can't put entire payload into a controlled environment as discussed in more detail in Section.~\ref{chap:GAPS:thermal:GCS}

\begin{figure}
    \centering
    \includegraphics[width=1\linewidth]{fig/GAPS-RAD-MEC-049_Foam_Section_33.png}
    \caption[GAPS payload styrofoam GAPS-RAD-MEC-049 Foam Section 33 design]{GAPS payload styrofoam GAPS-RAD-MEC-049 Foam Section 33 design. \textbf{(a)} Shows the design for this styrofoam piece which goes on top of the OHP heat transaction area. Small group of grooves are formed to divide all 36 loops of OHP tubings from tracker to radiator. \textbf{(b)} shows a 3D model of this particular piece. \textbf{(c)} shows the actual piece that I made with styrofoam and router. This design minimizes heat leakage from the tubing by cover all of the cold area.}
    \label{fig:foam_33}
\end{figure}
...
As discussed in Section.~\ref{chap:GAPS:detectors}, \ac{GAPS} detectors gain their peak performance under $ \sim -40^\circ~C $, which is designed to operate in the \ac{LDB} flight environment with dedicated thermal system. Before flight, the entire payload has to be calibrated to provide meaningful calibration measurements. For a small scale experiment, the solution is rather straightforward with a \ac{TVAC} testing that contains all the payload inside an environment-controlled chamber. But \ac{GAPS} payload exceeded $ \sim 6.6~m \times 4.1~m \times 4.6~m $ dimension, which makes it extremely hard to provide nominal \ac{TVAC} testing environment. One of the few possible facilities that could handle this type of task would be the Space Simulation Vacuum Chamber at the NASA’s Neil A. Armstrong Test Facility (ATF) in Sandusky, Ohio, but the cost of using it would be also way beyond budget because \ac{GAPS} would need multiple months to perform the detector calibration for more than 1000 channels on board \cite{nasa_glenn_sec}.
...
\begin{figure}
    ...
    \caption[GAPS \ac{GCS} design and mounting]{GAPS \ac{GCS} design and mounting. Left figure shows the schematics of \ac{GAPS} \ac{GCS} mounting on the payload. Right picture shows actual \ac{GAPS} \ac{GCS} mounted on the payload at MIT Bates lab.}
    \label{fig:GAPS_GCS_setup}
\end{figure}

To perform ground testing, the ground cooling system (\ac{GCS}) has been designed and assembled to provide flight-like thermal conditions, see Fig.~\ref{fig:GAPS_GCS_setup}. While at the same time all the detector modules are sealed with aluminum windows and purged with nitrogen gas, the low vacuum condition ($ \sim 3~torr $) throughout the entire payload is not achieved with the \ac{GCS} system. The \ac{GCS} cools the radiator and has a simple design. The payload configuration is not changed for testing, but non-flight items may be attached to the payload. A cold plate is attached to the radiator to cool it. The cold plate is tightened to the radiator with bolts uniformly distributed in the plate so that the two surface planes make good contact. A tube is attached to the cold plate, and a coolant flows through the tube, drawing heat from the plate. Once the radiator is sufficiently cooled, the HPS transfers heat from the detectors to the radiator, bringing the detectors down to operating temperature \cite{Hideyuki_FUKE20232}.

During \ac{GCS}-based cooldown of the \ac{GAPS} detector in preparation for the first flight, we implemented several improvements to enhance system performance and reliability. These improvements include optimizing the coolant flow rate, enhancing the thermal insulation around critical components, and implementing extra chillers to ensure consistent temperature control. These enhancements have significantly improved the efficiency of the cooling process, allowing for more stable and precise temperature regulation of the detectors, see Fig.~\ref{fig:GAPS_GCS}. Left figure shows schematics of \ac{GAPS} \ac{GCS} with 4 cold panels embedding 36 loops of methanol cooling-line manifold. Each 2 panels are cooled down by a \ac{ACC} circulating methanol. Right picture shows actual \ac{GAPS} \ac{GCS} setup at Columbia University Nevis Laboratory. Red arrows with letters show the methanol line displayed on the left.
\begin{figure}
    \centering
    \includegraphics[width=1\linewidth]{fig/GAPS_GCS.png}
    \caption[GAPS Ground Cooling System]{GAPS Ground Cooling System. Left figure shows schematics of \ac{GAPS} \ac{GCS} with 4 cold panels embedding 36 loops of methanol cooling-line manifold. Each 2 panels are cooled down by a \ac{ACC} circulating methanol. Right picture shows actual \ac{GAPS} \ac{GCS} setup at Columbia University Nevis Laboratory. Red arrows with letters show the methanol line displayed on the left.}
    \label{fig:GAPS_GCS}
\end{figure}
