...
The CSP v5 was calibrated with a charge injection board that makes use of a voltage divider, an injection capacitor of 1 pF and a dummy capacitor used to mimic the pGRAMS detector’s assumed 50 pF capacitance. The tests demonstrated that the CSP noise FWHM can be as low as 20~keV in room temperature. As Fig.~\ref{fig:CSP_detector_capacitance_effect} shows, we have changed the dummy detector capacitance to simulate possible detector tile strip capacitance. The result shows that the noise performance of the CSP is affected by the detector capacitance, which is expected. With lower detector capacitance, the noise performance is better, which in compare to pGRAMS detector strip capacitance of around 15 pF, we could achieve around 10 keV energy resolution. While under liquid argon cryogenic condition, the changing of the injection capacitor value is the unknown factor that we can't fully trust the conversion relation from equation.~\ref{equ:Charge_conversion}. The result in this figure only shows the asumption that the conversion relation is the same at cryogenic temperature, which is still under testing.

\begin{figure}
    \centering
    \includegraphics[width=1\linewidth]{fig/CSP_detector_capacitance_effiect.png}
    \noindent\raggedright 
    A series of bench test for \ac{CSP} noise performance with different dummy capacitance. The result shows that the noise performance of the CSP is affected by the detector capacitance, which is expected. With lower detector capacitance, the noise performance is better, which in compare to pGRAMS detector strip capacitance of around 15 pF, we could achieve around 10 keV energy resolution. While under liquid argon cryogenic condition, the changing of the injection capacitor value is the unknown factor that we can't fully trust the conversion relation from equation.~\ref{equ:Charge_conversion}. The result in this figure only shows the asumption that the conversion relation is the same at cryogenic temperature, which is still under testing.
...
\begin{figure}
    ...
    \noindent\raggedright \textbf{(a)} Left figure shows cryostat general view. miniGRAMS TPC is mounted inside the inner cryostat. There are kapton heaters installed outside of inner cryostat to help release the argon before descending. \textbf{(b)} Middle top figure shows area that between inner cryostat and outer vacuum jacket. Green cylinders are G10 supporting rods for the purpose of low thermal conductance. Thin blue/red/yellow hoses are plumbing lines for fill/vent/\ac{LLD} respectively. middle-siezd yellow line shows safety rapture disk. \textbf{(c)} middle bottom figure shows bottom of the cryostat. center KF cross is used for plumbing, while each quadrant has a signal flange for charge and light readout, \ac{HK} sensors, detector power flange and high voltage flange. \textbf{(d)} Right figure shows the actual pGRAMS cryostat hanging on the stand at \ac{GSFC}.
...
As for the power source we have selected battery stack SR182175\cite{nasa_csbf_sr182175} which was widely used in balloon experiments. Each battery stack has a nominal voltage of 26V (22V - 30V depending on the load) and voltage capacity 50 Ah. With our power budget mentioned in Table.~\ref{tab:pGRAMS_powerbudget}, we could estimate the mission time based on the number of battery stacks. With an total nominal load of 293.8375W and \ac{PDU} efficiency of 85\%, we could estimate the current and flight time for different number of battery stacks. Table.~\ref{tab:pGRAMS_flight_time} shows the estimated flight times for different scenarios. Per battery stack's manual, it could behave as worse as output 22V with load that is not ideal while each battery stacks will have lower probability to fail with lower current. Based on the flight requirement of minimum 12 hours, we will need at least 4 battery stacks to meet the requirement. With 4 battery stacks, we could have around 15 hours of flight time with nominal load, and around 13 hours of flight time with worst load. With more battery stacks, we could have more margin for the flight time, but it will also add more weight to the payload which will affect the flight altitude and also create mechanical stress on the gondola. We will need to balance the flight time and weight to make sure we have a successful flight.
...
\end{tabular*}
\noindent\raggedright Top chart shows the flight time estimation for a nominal battery performance of outputing rated 28V with load, while bottom chart shows the flight time estimation for a worst battery performance of outputing 22V with load. With current limit 5A for each battery stack, all of these cases would be within the limit. 
