...
To deal with the peak shift, I have developed a peak shift correction algorithm to further correct the peak location. After the gaussian smoothing, peak location correction is performed to make the peak location more precise. The peak correction result is showed in Fig.~\ref{fig:peak_correction}

\begin{figure}
    \centering
    \includegraphics[width=1\linewidth]{fig/peak_shift.png}
    \noindent\raggedright 
...
From Section.~\ref{chap:intro:balloon} we know that for balloon experiment, payload weight and power budget are crucial. In order to get the most efficient detector structure with limited mass budget in hand, I have conducted a simulation study to determine the shape of the detector under a series of fixed total weight budget. In this study, I have made the assumption that \ac{PDU}, electronics, and other support systems will not be affected by the detector geometry change, and the mass budget for these systems is fixed. Therefore, the total mass budget for the detector system including \ac{LArTPC} and \ac{TOF} system will be the major element to determine the detector geometry. Chart in table.~\ref{tab:grams_mass_budget} shows the detailed mass breakdown for the total 1270 kg detector mass budget which comes from the preliminary mass estimation for the GRAMS balloon payload based on historical balloon experiments. With this study, I did a series of simulations to evaluate the GRASP that using the same method as section.~\ref{chap:GRAMSresult:antihelium-3:GRASP} mentioned, for a mass list of 900 kg, 1100 kg, 1300 kg and 1500 kg, fine tuning the liquid argon geometry to find the optimal shape for the detector. Table.~\ref{tab:LArTPC_geometry} shows the detailed geometry configuration for the liquid argon detector under different total mass budget. The length of the liquid argon detector is varied from 50 cm to 600 cm, and the thickness is varied accordingly to fit the total mass budget. The red numbers in the table indicate the configurations where the liquid argon mass becomes negative, which is not physical and therefore not considered in the optimization. With the geometry configuration for each total mass budget, we could then evaluate the GRASP for each configuration to find the optimal shape for the detector under different mass budget. For each configuration, 1 million antihelium-3 events are simulated with the GEANT4 package, and the GRASP is calculated based on the number of events that can be successfully captured.
...
\begin{figure}
    ...
    \noindent\raggedright Y-axis representating the capture power of the detector torwards antihelium-3 nuclei, which is calculated as the number of captured events divided by the total number of simulated events. Based on the results, the efficiency reached maximum at a more cubic shape comparing to thin flat pancake shape and long rod shape.
    \caption{GRAMS \ac{LArTPC} GRASP result with different geometry configuration under fixed total mass budget.}
    \label{fig:GRAMS_LArTPC_shape_geo}
\end{figure}
From Fig.~\ref{fig:GRAMS_LArTPC_shape_geo}, we can see that with larger detector mass budget, the capture efficiency is higher as expected. And the efficiency peak also shift to the right. Based on this result, \ac{GRAMS} has proposed the detector shape to be $ 140cm \times 140cm \times 20cm $ with 1270kg of detector mass budget. Checked with the GRASP result, 140cm lays between 140cm peak from 1100kg mass and 160cm TPC length from 1300kg mass, which is still an optimal choice for the detector geometry. 

Aside from the fixed total mass budget of the \ac{LArTPC} and TOF system, I also did a simulation study of the \ac{LArTPC} depth study to evaluate the GRASP with different \ac{LArTPC} depth under the same length and width. This will tell us the events distribution inside our detector to give us better understanding of the detector performance and help us to optimize the detector design. Fig.~\ref{fig:GRAMS_LArTPC_depth} shows the stopped events' GRASP depth distribution for antideuterons. We can see that most of the events are captured within the first 10cm of the detector, which is consistent with the fact that most of the annihilation products are generated within the first few centimeters of the detector. This result also indicates that we can optimize the detector design by focusing on the first 10cm of the detector to maximize the capture efficiency while minimizing the total mass budget.

One major reason that that the GRASP depth distribution has this shape is that the low energy events are annihilated in the atmosphere and only for energy higher than $ 75keV/N $ started to reach the detector. And for events that are higher energy than $ 200keV/N $, they tend to escape from the detector without being captured. The tails of high energy events are the ones with large zelith angle with give a very long trajactory inside the \ac{LArTPC}. But those events are relatively small within all events.
